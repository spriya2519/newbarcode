...
export default function ItemTable({ user }) {
  ...
  // --- Barcode Generators ---
  const generateFullBarcode = (row) =>
    `${row.partno}-${row.desc}-${row.qty}-${row.gr_no}-${
      row.accepted ? "ACCEPTED" : "REJECTED"
    }`;

  const generateIdBarcode = (row) => `${row.partno}`;
                ...
                <TableCell>
                  {row.comment && row.accepted !== null && (
                    <Box display="flex" flexDirection="column" gap={1}>
                      {/* Barcode 1 → Full product details */}
